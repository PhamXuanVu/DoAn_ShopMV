			...
			<div class="row">
				...
				<div class="col-md-7">
					...
					<c:forEach items="${sanPham.chiTietSanPham.kichCos}" var="k">
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" checked="checked"
								name="kichCo" id="inlineRadio1" value="${k.tenKichCo }" /> <label
								class="form-check-label" for="inlineRadio1">${k.tenKichCo }</label>
						</div>
					</c:forEach>
					<br />
					<b>Màu: </b> <br />
					<c:forEach items="${sanPham.chiTietSanPham.mauSacs}" var="m">
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" checked="checked"
								name="mauSac" id="inlineRadio1" value="${m.tenMau }" /> <label
								class="form-check-label" for="inlineRadio1">${m.tenMau }</label>
						</div>
					</c:forEach>
					<P class="price">${sanPham.getGiaFormat()}</P>
					<label>Số lượng:</label> <input name="soLuong" type="number"
						value="1" min="1" max="${sanPham.soLuong}"> &nbsp
					${sanPham.soLuong} sản phẩm có sẵn <br /> <br />
					<c:choose>
						<c:when test="${sanPham.soLuong == 0}">
							<button type="submit" disabled="disabled"
								class="btn btn btn-secondary cart">Hết Hàng</button>
						</c:when>
						<c:otherwise>
					...
					<p class="small fw-bold mt-2 pt-1 mb-0">
						Xem các sản phẩm khác của cửa hàng <a
							href="${pageContext.request.contextPath}/danhmuc/san-pham-cua-hang/${sanPham.cuaHang.cuaHangId}"
