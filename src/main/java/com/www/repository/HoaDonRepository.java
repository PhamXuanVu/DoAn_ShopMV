...
public interface HoaDonRepository extends CrudRepository<HoaDon, Integer> {
	@Query(name = "select * from hoa_don where nguoiDungId = ?1", nativeQuery = true)
	List<HoaDon> findHoaDonsByNguoiDungId(int id);
	
	@Query(name = "select * from hoa_don h join chi_tiet_san_pham_hoa_don c on c.hoa_don_san_pham_id= h.hoaDonId where c.cuaHangId =?1", nativeQuery = true)
